...
.DESCRIPTION
    Monitors crypto-enhanced trading bot for process health, trade activity,
    error patterns, and system resources.
...
function Send-BotNotification {
    ...
    if ($script:UseBurntToast) {
        try {
            ...
            New-BurntToastNotification -Text $Title, $Message -Sound $sound -AppLogo "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -ErrorAction SilentlyContinue
        }
        catch {
...
function Start-BotProcess {
    ...
    try {
        ...
        $processArgs = @{
            ...
            ArgumentList = @("-NoExit", "-Command", "cd '$botDir'; & '$($Config.VenvActivate)'; echo YES | python $script")
        ...
        while ($true) {
            ...
            if (-not $botProcess) {
                ...
                if ($memoryMB -gt $Config.MemoryThresholdMB) {
                    Send-BotNotification -Title "⚠️ High Memory Usage" -Message "Bot using ${memoryMB}MB (threshold: $($Config.MemoryThresholdMB)MB)" -Severity Warning
                }
            }
            ...
            if (($now - $lastTradeCheck).TotalMinutes -ge $Config.NoTradeAlertMinutes) {
                ...
                if ($lastTrade) {
                    ...
                    if ($minutesSinceLastTrade -gt $Config.NoTradeAlertMinutes) {
                        Send-BotNotification -Title "⏰ No Recent Trades" -Message "Last trade was $minutesSinceLastTrade minutes ago. Possible WebSocket disconnect?" -Severity Warning
                    }
                }
